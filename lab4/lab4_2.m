...
spread = 0.3;
goal = 1e-5;
network = newrb([X(Points(trainSet)); Y(Points(trainSet))], Target(:, trainSet), ...
    goal, spread);
view(network);

PredictedClasses = network([X(Points(trainSet)); Y(Points(trainSet))]) >= 0.5;
...
PredictedClasses = network([X(Points(testSet)); Y(Points(testSet))]) >= 0.5;
...
Ans = network([RegionX(:)';RegionY(:)']) >= 0.5;
Ans = max(0, min(1, Ans));
Ans = round(Ans * 10) * 0.1;
